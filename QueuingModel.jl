...
in_priors = Dict(
    "r" => LogNormal(μ_ln(5,2.0), σ_ln(5,2.0)), 
    "p" => Beta(2, 500), 
...
@time chain = sample(queuing_model(sum(counts_2_month,dims=2),counts_2_month, input_data, times_vec,
...
quantiles_N0 = stack([confidence_intervals(t->sample_model_queuing(chain_df,t)[1],t,q_levels = [0.025,0.1, 0.25,0.5,0.75, 0.9, 0.975],N_samples=N_samples) for t in t_vals])
quantiles_N1 = stack([confidence_intervals(t->sum(sample_model_queuing(chain_df,t)[2:3]),t,q_levels = [0.025,0.1, 0.25,0.5,0.75, 0.9, 0.975],N_samples=N_samples) for t in t_vals])
quantiles_N2 = stack([confidence_intervals(t->sum(sample_model_queuing(chain_df,t)[4:5]),t,q_levels = [0.025,0.1, 0.25,0.5,0.75, 0.9, 0.975],N_samples=N_samples) for t in t_vals])
...
sample_fun  = t-> hidden_states(sample_model_queuing(chain_df,t))
...
p_θ345 = histogram(chain_df.θ345,normalize=:pdf,xlabel="θ345",label="Posterior", grid=false)
plot!(p_θ345, 0:0.01:1,pdf(in_priors["θ345"],0:0.01:1),label = "Prior")

p_θ678 = histogram(chain_df.θ678,normalize=:pdf,xlabel="θ678",label="Posterior", grid=false)
plot!(p_θ678, 0:0.01:1,pdf(in_priors["θ678"],0:0.01:1),label = "Prior")

p_θ67 = histogram(chain_df.θ67,normalize=:pdf,xlabel="θ67",label="Posterior", grid=false)
plot!(p_θ67, 0:0.01:1,pdf(in_priors["θ67"],0:0.01:1),label = "Prior")

p = plot_π_posterior(chain,in_priors)
plot(p...,p_r,p_p,p_w1,p_w2,p_w3,p_θ12,p_θ34,
    p_θ345,p_θ678,p_θ67,
    layout = (4,4), size=(1400,800), margin = 4mm,ylabel="density")
savefig("plots/queuing_model_prior_posterior.pdf")